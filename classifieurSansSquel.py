...
import scipy.stats
import Elongation
from handDetect import HandsLandmarks, HandsLandmarksSansExtreme
...
def apprentissage():
    ...
    for imgpath in glob.glob('Apprentissage/*.bmp'):
        ...
        att3.append(HandsLandmarksSansExtreme(imgpath))
    model_histGrad = HistGradientBoostingClassifier()
    model_Gaussian = GaussianNB()
    Xtrain = np.zeros((12*taille,49))
...
#    model_Gaussian.fit(Xtrain,Ytrain)
    ...
    for imgpath in glob.glob('Test/*.bmp'):
        ...
        att3_test.append(HandsLandmarksSansExtreme(imgpath))
    Xtest = np.zeros((12*(13-taille),49))
